...
void RenumberDlg::SetDefaults(void)
{
    ...
    CheckRadioButton(IDRB_LFT_RGT_TOP_BOT, IDRB_RGT_LFT_BOT_TOP, IDRB_LFT_RGT_TOP_BOT);
...
void RenumberDlg::GetSelectedComponents(void)
{
    ...
    itoa(nSelComps, status_dlg.cBufr, 10);
    status_dlg.sStatus = CString("Reading selected components (") + status_dlg.cBufr + ")";
    status_dlg.SetActiveWindow();
    status_dlg.SetWindowText( "Renumber Components" );
    status_dlg.mTextStatus.SetWindowText( status_dlg.sStatus );
        ...
        if (selected_item.itemType == DBX_COMPONENT)
        {
            ...
            *ndx = '\0';
            if (pPrefsIndices->Find(refdes) == NULL)
            {
                ...
                itoa(nSelComps, status_dlg.cBufr, 10);
                status_dlg.sStatus = CString("Reading selected components (") + 
                                                status_dlg.cBufr + ")";
                status_dlg.mTextStatus.SetWindowText(status_dlg.sStatus);
            }
        }
...
void RenumberDlg::RenumberComponents(void)
{
    ...
    nRenComps = 0;
    StatusDlg status_dlg;
    status_dlg.sStatus = CString("Renumbering selected components (1)");
    status_dlg.SetActiveWindow();
    status_dlg.SetWindowText( "Renumber Components" );
    status_dlg.mTextStatus.SetWindowText( status_dlg.sStatus );
...
//                char *ref_des = p_item->GetRefDes();
                ...
                if ((IsCompInThisPass(p_item)) == TRUE)
                {
                    ...
                    if (tStatus == DBX_OK)
                    {
                        ...
                        if ((nRenComps % 3) == 0)                  
                        {
                            itoa(nRenComps, status_dlg.cBufr, 10);
                            status_dlg.sStatus = CString("Renumbering selected components (") + 
                                            status_dlg.cBufr + ")";
                            status_dlg.mTextStatus.SetWindowText(status_dlg.sStatus);
                        }
                    }
...
void RenumberDlg::SortComponents(void)
{
    ...
    StatusDlg status_dlg;
    status_dlg.sStatus = CString("Sorting components list (1)");
    status_dlg.SetActiveWindow();
    status_dlg.SetWindowText( "Renumber Components" );
    status_dlg.mTextStatus.SetWindowText( status_dlg.sStatus );
    ...
    for(int indx = 0;  indx < nSelComps;  indx++)
    {
        ...
        if ((indx % 3) == 0)                  
        {
            itoa(indx, status_dlg.cBufr, 10);
            status_dlg.sStatus = CString("Sorting components list (") + status_dlg.cBufr + ")";
            status_dlg.mTextStatus.SetWindowText(status_dlg.sStatus);
        }
    }   // for(int indx
    listSorted = TRUE;
    ...
    if (tStatus == DBX_OK  &&  comp.compId != pCompItem->GetCompId())
    {
        ...
        do
        {
            ...
            if (tStatus == DBX_OK)
            {
                if (!in_complist)
                {
                    // this one not in master list - put it in the list of components which have
                    // been given temp[orary refDes's
