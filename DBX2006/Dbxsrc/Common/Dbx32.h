...
//
//    $Header: Dbx32.h, 2, 1/31/06 11:29:00 AM, Andy Ball$
//                                                                          
...
#define DBX_INVALID_LAYER_BIAS      32202  // New for v18.0
#define DBX_INVALID_VARIANT_NAME    32203
...
//
#define DBX_VERSION                 18000   // Currently working on 16.000 
...
#define DBX_RULE_DRC_NON_PLATED_HOLE_NO_CONNECT       108
#define DBX_RULE_DRC_NON_PLATED_THROUGH_HOLE          109
...
{
    long            styleId;
    long            styleType;                   // DBX_PAD_STYLE or DBX_VIA_STYLE 
...
{                                                
    char            layerName[DBX_MAX_NAME_LEN]; // pcb  sch
    char            layerMaterial[DBX_MAX_NAME_LEN];
    char            layerThickness[DBX_MAX_NAME_LEN];
    char            layerDialectricConstant[DBX_MAX_NAME_LEN];
...
{
    char            name[DBX_MAX_NAME_LEN];
    char            description[DBX_MAX_TEXTITEM_LEN];
    long            id;
} TVariant;
...
{                   
    ...
    long            numSides;       // side count
    long            rotAngle;       // rotation
    long            circDia;        // circumscribed diameter
    long            isPourNoConn;   // Prohibit Cu pour thermalizing
} TPadViaRegularPolyShape;
...
//
//  pContext            DbxContext*  Input DBX conversation data
//  pLayer              TLayer*      Layer to add.  If layer number is 0 or already exists system auto changes number to next available.
//                                   If layer name already exists error is returned.
//
//  Returns             long         DBX completion status
//
...
//
//  pContext            DbxContext*  Input DBX conversation data
//  pLayer              TLayer*      Sheet to add.  If sheet number is 0 or already exists system auto changes number to next available.
//                                   If sheet name already exists error is returned.
//
//  Returns             long         DBX completion status
//
long 
DLLX TAddVariant(DbxContext* pContext, TVariant* pVariant);
// TAddVariant- (Pcb/Sch) Add a variant to the given design.
// -----------------   
//
//  parameter           Type/Description
//  ---------           ----------------
//
//  pContext            DbxContext*  Input DBX conversation data
//  pVariant            TVariant*    Variant to add.
//                                   If variant name already exists error is returned.
//
//  Returns             long         DBX completion status
//
long 
DLLX TDeleteVariant(DbxContext* pContext, TVariant* pVariant);
// TDeleteVariant- (Pcb/Sch) Remove a variant from the given design.
// -----------------   
//
//  parameter           Type/Description
//  ---------           ----------------
//
//  pContext            DbxContext*  Input DBX conversation data
//  pVariant            TVariant*    Variant to remove.
//                                   If variant does not exist an error is returned.
//
//  Returns             long         DBX completion status
//
long 
DLLX TRenameVariant(DbxContext* pContext, TVariant* pVariant);
// TRenameVariant- (Pcb/Sch) Remove a variant from the given design.
// -----------------   
//
//  parameter           Type/Description
//  ---------           ----------------
//
//  pContext            DbxContext*  Input DBX conversation data
//  pVariant            TVariant*    Variant to rename.
//
//                      Use the variant id to specify which variant should be renamed.
//                      Use the variant name for specifying the new name.
//                      If variant id is outside of range an error is returned.
//
//  Returns             long         DBX completion status
//
long 
DLLX TGetFirstVariant(DbxContext* pContext, TVariant* pVariant);
// TGetFirstVariant- (Pcb/Sch)  Retrieve the first variant from the given design.
// -----------------   
//
//  parameter           Type/Description
//  ---------           ----------------
//
//  pContext            DbxContext*  Input DBX conversation data
//  pVariant            TVariant*    Retrieved variant - may be null.
//
//  Returns             long         DBX completion status
//
long
DLLX TGetNextVariant(DbxContext* pContext, TVariant* pVariant);
// TGetFirstVariant- (Pcb/Sch)  Retrieve the first variant from the given design.
// -----------------   
//
//  parameter           Type/Description
...
//
// TGetFirstLayer - (PCB) Return data for the first layer stackup in the design.
// --------------
//
//  parameter     Type/Description
//  ---------     ----------------
...
long
DLLX TGetFirstSymbol  (DbxContext*  pContext,    // (i/o)  dbx context
                       TSymbol*     pTSymbol);   // (i/o)  output symbol
// TGetFirstSymbol - (Sch) Get data for first symbol.
// ------------------
//
//  parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*  Input DBX conversation data
//  pTSymbol      TSymbol*     Output symbol item
//
//
//  Returns       long         DBX completion status
//
...
DLLX TGetNextSymbol   (DbxContext*  pContext,
                       TSymbol*     pTSymbol);
//
// TGetNextSymbol      - (Sch) Get the next symbol.  The TGetFirstSymbol is called
//                     before this
// ---------------------          
//
//  parameter           Type/Description
...
//
//  pContext            DbxContext*  Input DBX conversation data
//  pTSymbol            TSymbol*     Output symbol
//
//  Returns       long         DBX completion status
//
...
long  
DLLX TCreatePadStyle(DbxContext*   pContext,   // (i/o) dbx context info
                     long          fromId,     // (i)   existing style Id to copy from - Use 0 for default
                     const char*   pNewName,   // (i)   new style name
                     TPadViaStyle* pStyle);    // (o)   new style return

//
//  TCreatePadStyle - (PCB) Create a new pad style by copying from the 
//                          existing style id in the design
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  fromId        long          The existing pad style Id in the design to copy from. Id 0
//                              is the default style in the design.  You can use the
//                              TGetFirstPadStyle and TGetNextPadStyle functions to 
//                              fetch the existing pad styles in the design.
//  pNewName      const char*   New style name
//  pStyle        TPadViaStyle* If successful, the function returns the new style properties
//                              in this parameter.  It fills in the style id, name, hole
//                              diameter, etc...See TPadViaStyle type definition for more details.
//
//  returns       long          DBX_OK if successfull, otherwise the following possible errors
//                              is returned:
//
//                              DBX_BAD_INPUT       - Some of the parameters passed-in are invalid or
//                                                    the new name might be too long
//                              DBX_ITEM_NOT_FOUND  - The fromId does not exist in the design
//                              DBX_INTERNAL_PARAMETER_ERROR - Server error when creating new style
//

long
DLLX TDeletePadStyle(DbxContext* pContext,      // (i/o) dbx context info
                     long        padStyleId);   // (i)   pad style Id to delete
//
//  TDeletePadStyle - (PCB) Delete pad style from the design.  Default style or styles that
//                          are in-use in the design cannot be deleted.
//
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  padStyleId    long          The pad style id to delete.
//
//  returns       long          DBX completion status
//
//  Note: Default styles or styles that are in-use cannot be deleted
//

long 
DLLX TAddPadShape(DbxContext*    pContext,      // (i/o) dbx context info
                  long           padStyleId,    // (i) Pad style Id to add shape to
                  TPadViaShape*  pPadShape);    // (i) Pad shape to add
//
//  TAddPadShape - (PCB) Add or modify the pad shape (for a layer) of a pad style.  
//                       For polygon pad shape, use the TAddPolyPadShape function
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  padStyleId    long          The pad style id to to add or modify
//  pPadShape     TPadViaShape* Use the structure to pass in the Layer Id for the 
//                              pad shape, shape type such as oval, rectange, etc...
//                              
//  returns       long          DBX completion status
//
//  Note: Because polygon shape uses different property attributes, this function
//        will not add polygon pad shape.  To add/modify polygon pad shapes for a
//        layer, use the TAddPolyPadShape function
//

long
DLLX TAddPadRegularPolyShape(DbxContext*              pContext,        // (i/o) dbx context info
                             long                     padStyleId,      // (i)   pad style id
                             TPadViaRegularPolyShape* pRegPolyShape);  // (i)   Regular poly shape definition
//
//  TAddPadRegularPolyShape - (PCB) Add or modify the regular polygon shape for a pad style
// 
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*                Input DBX conversation data
//  padStyleId    long                       The pad style id to modify
//  pRegPolyShape TPadViaRegularPolyShape*   Pass in the number of sides, rotation angle for 
//                                           the regular polygon
//
//  returns       long                       DBX completion status
//
...
long
DLLX TAddPadIrregularPolyShape(DbxContext*   pContext,           // (i/o) dbx context info
                               long          padStyleId,         // (i)   pad style id
                               long          layerId,            // (i)   layer Id or 0 if add or modify default layer
                               long          layerType,          // (i)   use if layer Id is 0
                               long          isPourNoConn,       // (i)   Prohibit Cu pour thermalizing
                               long          numberOfPoints,     // (i)   number of poly points
                               const TCoord* pPoints);           // (i)   point array. The size of the array is numberOfPoints
//
//  TAddPadIrregularPolyShape - (PCB) Add or modify the Irregular polygon shape for a pad style.
//
//
//  Note: The array of points, the pPoints parameter, defines polygon shape.  The X,Y coordinate
//        of a point is relative to the "origin" of the polygon, not the PCB design.  The 
//        coordinate is in database unit.  
//
//        The layer Id is the layer number for the pad shape.  If the layer number is 0, the layerType
...
long
DLLX TDeletePadShape(DbxContext* pContext,   // (i/o) dbx context info                     
                     long        padStyleId, // (i) pad style id
                     long        layerId);   // (i) layer id
//
//  TDeletePadShape - (PCB) Delete a pad style shape.
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  padStyleId    long          The pad style id to modify
//  layerId       long          The layer Id for the pad shape to be removed
//                              
//  returns       long          DBX completion status
//
//  Note: Cannot delete default shapes for the following layers: top, bottom, 
//        default non-signal, signal and plane.
//

long  
DLLX TCreateViaStyle(DbxContext*   pContext,   // (i/o) dbx context info
                     long          fromId,     // (i)   existing style Id to copy from - Use 0 for default
                     const char*   pNewName,   // (i)   new style name
                     TPadViaStyle* pStyle);    // (o)   new style return
//
//  TCreateViaStyle - (PCB) Create a new via style by copying from the 
//                          existing style id in the design
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  fromId        long          The existing via style Id in the design to copy from. Id 0
//                              is the default style in the design.  You can use the
//                              TGetFirstViaStyle and TGetNextViaStyle functions to 
//                              fetch the existing via styles in the design.
//  pNewName      const char*   New style name
//  pStyle        TPadViaStyle* If successful, the function returns the new style properties
//                              in this parameter.  It fills in the style id, name, hole
//                              diameter, etc...See TPadViaStyle type definition for more details.
...
long
DLLX TDeleteViaStyle(DbxContext* pContext,      // (i/o) dbx context info
                     long        viaStyleId);   // (i)   via style Id to delete
//
//  TDeleteViaStyle - (PCB) Delete via style from the design.  Default style or styles that
//                          are in-use in the design cannot be deleted.
//
//
//  Parameter     Type/Description
...
//
//  pContext      DbxContext*   Input DBX conversation data
//  viaStyleId    long          The via style id to delete
//
//  returns       long          DBX completion status
//
//  Note: Default styles or styles that are in-use cannot be deleted
...
long 
DLLX TAddViaShape(DbxContext*    pContext,      // (i/o) dbx context info
                  long           viaStyleId,    // (i)   Via style Id to add shape to
                  TPadViaShape*  pViaShape);    // (i)   via shape to add
//
//  TAddViaShape - (PCB) Add or modify the via shape for a via style.  
//                       For polygon via shape, use the TAddPolyViaShape function
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  viaStyleId    long          The via style id to to add or modify
//  pViaShape     TPadViaShape* Use the structure to pass in the Layer Id for the 
//                              via shape, shape type such as oval, rectange, etc...
//                              
//  returns       long          DBX completion status
//
//  Note: Because polygon shape uses different property attributes, this function
//        will not add polygon via shape.  To add/modify polygon via shape for a
//        layer, use the TAddPolyViaShape function
//

long
DLLX TAddViaRegularPolyShape(DbxContext*               pContext,        // (i/o) dbx context info
                             long                      viaStyleId,      // (i)   via style id
                             TPadViaRegularPolyShape*  pRegPolyShape);  // (i)   Regular poly shape definition
//
//  TAddViaRegularPolyShape - (PCB) Add or modify the regular polygon shape for a via style
// 
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*               Input DBX conversation data
//  viaStyleId    long                      The via style id to modify
//  pRegPolyShape TPadViaRegularPolyShape*  Pass in the number of sides, rotation angle for 
//                                          the regular polygon
//
//  returns       long                      DBX completion status
...
long
DLLX TAddViaIrregularPolyShape(DbxContext*   pContext,           // (i/o) dbx context info
                               long          viaStyleId,         // (i)   via style id
                               long          layerId,            // (i)   layer Id or 0 if add or modify default layer
                               long          layerType,          // (i)   use if layer Id is 0
                               long          isPourNoConn,       // (i)   Prohibit Cu pour thermalizing
                               ...
                               const TCoord* pPoints);            // (i)  point array.  The size of the array is numberOfPoints
//
//  TAddViaIrregularPolyShape - (PCB) Add or modify the Irregular polygon shape for a via style.
//
//  returns       long                        DBX completion status
//
//  Note: The array of points, the pPoints parameter, defines polygon shape.  The X,Y coordinate
...
//        coordinate is in database unit.  
//
//        The layer Id is the layer number for the via shape.  If the layer number is 0, the layerType
//        can be used to modify the default shape of a via style:
//             DBX_LAYERTYPE_SIGNAL      -> shape for default Signal Layer
//             DBX_LAYERTYPE_PLANE       -> shape for default Plane Layer, and
//             DBX_LAYERTYPE_NON_SIGNAL  -> shape for default Non Signal layer
//
...
long
DLLX TDeleteViaShape(DbxContext* pContext,   // (i/o) dbx context info                     
                     long        viaStyleId, // (i) via style id
                     long        layerId);   // (i) layer id

//
//  TDeleteViaShape - (PCB) Delete a via style shape.
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  viaStyleId    long          The via style id to modify
//  layerId       long          The layer Id for the via shape to be removed
//                              
//  returns       long          DBX completion status
//
//  Note: Cannot delete default shapes for the following layers: top, bottom, 
...
long
DLLX TModifyPadStyle(DbxContext*   pContext,  // (i/o) dbx context info                     
                     TPadViaStyle* pStyle);   // (i)   style info to modify
//
//  TModifyPadStyle - (PCB) Modify a pad style
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  pStyle        TPadViaStyle* The pad style info to modify.  Need to fill the following fields:
//                                   - styleId   
//                                   - name      
//                                   - holeDia   
//                                   - xOffset
...
long
DLLX TModifyViaStyle(DbxContext*   pContext,  // (i/o) dbx context info                     
                     TPadViaStyle* pStyle);   // (i)   style info to modify
//
//  TModifyPadStyle - (PCB) Modify a via style
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*   Input DBX conversation data
//  pStyle        TPadViaStyle* The via style info to modify.  Need to fill the following fields:
//                                   - styleId   
//                                   - name      
//                                   - holeDia   
//                                   - xOffset
...
long  
DLLX TAddTextStyle(DbxContext*   pContext,   // (i/o) dbx context info
                   long          fromId,     // (i)   existing style Id to copy from
                   TTextStyle*   pStyle);    // (i/o) new text style return
//
//  TAddTextStyle - (PCB/SCH) Add a new text style by copying it from an existing style
//
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*  Input DBX conversation data
//  fromId        long         The existing style to copy from
//  pStyle        TTextStyle*  The style to receive when return.
//                             Field must be filled in on input:
//                                 - name   (The new name of the style)
//
//  returns       long         DBX completion status
//
//  Note:  Default style or style that are in-used cannot be deleted.
//

long  
DLLX TDeleteTextStyle(DbxContext*   pContext,   // (i/o) dbx context info
                      long          styleId);   // (i)   style to delete                  
//
//  TDeleteTextStyle - (PCB/SCH) Delete an existing text style
//
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*  Input DBX conversation data
//  styleId       long         The style Id to be deleted
//
//  returns       long         DBX completion status
//
//  Note:  Default style or style that are in-used cannot be deleted.
...
long
DLLX TModifyTextStyle(DbxContext* pContext,  // (i/o) dbx context info
                      TTextStyle* pStyle);   // (i)   style info to modify
//
//  TModifyTextStyle - (PCB/SCH) modify the input DBX Net
//
//
//  Parameter     Type/Description
//  ---------     ----------------
//
//  pContext      DbxContext*  Input DBX conversation data
//  pStyle        TTextStyle*  The style Id field is the text style to be modified
//                             Valid fields to modify:
//                                  - name                
//                                  - strokeHeight
//                                  - strokePenWidth
...
//                                  - tTypeHeight 
//
//  returns       long         DBX completion status and pStyle updated to reflect changes
//
//  Note: This function returns DBX_BAD_INPUT when one of the following possible errors happened:
//             - The name is found as duplicated as other style names.
//             - Height or width of the text are out of range
//
//        This function returns DBX_ILLEGAL_OP when one of the following possible errors happened:
//             - The style to be modifed is a default style (i.e. (Default), (DefaultTTF))
//             - The new font height and width might make the text fall out of the workspace
//
